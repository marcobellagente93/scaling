...
def test_flash_attention_training(
    tmp_path: Path,
    ...
    return_dict_resumed_trained_model_with_flash_attn = dist_launcher(
        run_func=run_test_training,
        world_size=world_size,
        master_port=find_free_port(),
    ...
    for loss_original, loss_resumed in zip(
        ...
        return_dict_resumed_trained_model_with_flash_attn["metrics"],
    ):
        diff_pct = abs(loss_original["training/loss"] - loss_resumed["training/loss"]) / loss_original["training/loss"]
        assert (
            diff_pct < 1e-1  # TODO: Why is such huge loss difference needed here? This can't just be numerics...
        ), (
            f"loss changed after continuing training from a checkpoint with flash attention; "
            f"expected {return_dict_continuously_trained_model['metrics'][-4:]}, "
            f"got {return_dict_resumed_trained_model_with_flash_attn['metrics']}; diff_pct {diff_pct}"
        )
